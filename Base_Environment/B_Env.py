...
if Mod_Work == False:
    ...
    ldir=fr"D:\Twink\Benchmark_Modules\AWM"

from PyQt5.QtWidgets import QComboBox, QCompleter
from PyQt5.QtCore import QSortFilterProxyModel,Qt,QCoreApplication,QDate
...
@Exception_Handle
def DB_Exe(dbc,sql,prnt):
    if prnt == True:
        print(sql)
    dbc.execute(sql)

@Exception_Handle
def DB_Cmt(dbc,db,sql,prnt):
    if prnt == True:
        print(sql)
    dbc.execute(sql)
    db.commit()
...
@Exception_Handle
def DB_Fetch(dbc,sql,prnt,type):
    if prnt == True:
        print(sql)
    dbc.execute(sql)
    if type == "LOE":
...
@Exception_Handle
def DB_Push_Dict(dbc,db,dict,tbl_nm,prnt):

    placeholders = ', '.join([f'"{element}"' for element in dict.values()])
    columns = ', '.join(dict.keys())
    sql = f"INSERT INTO {tbl_nm} ( {columns} ) VALUES ( {placeholders} )"

    DB_Cmt(dbc,db,sql,prnt)

@Exception_Handle
def DB_Update_Dict(dbc,db,dict,c_dict,tbl_nm,prnt):

    dict =', '.join([f"{key} = '{value}'" for key, value in dict.items()])
    c_dict = ', '.join([f"{key} = '{value}'" for key, value in c_dict.items()])
    sql = f" UPDATE {tbl_nm} SET {dict} WHERE ({c_dict})"
    DB_Cmt(dbc, db, sql, prnt)


@Exception_Handle
def Use_Root_Dir():#CMD to Use Root Dir
    os.chdir(ldir)
...
# ___QT Functions___
@Exception_Handle
def Fetch_Table_Values(inp):#___Fetch_Table_Values___
    values = []
    for row in range(inp.rowCount()):
        ...
        for column in range(inp.columnCount()):
            ...
            if item is not None:
                ...
                if str(tempitem)[17:25] == "QSpinBox":
...
def Adjust_Table_Width(element,col_width):
    for i in range(len(col_width)):
        element.setColumnWidth(i,col_width[i])
...
def Filter_Table_Data(element,src_txt,data,col):
    ...
    temp=[]
    for step in data:
        if src_txt.lower()in step[col].lower():
            temp.append(step)
    element.setSortingEnabled(False)
...
def Fetch_Employee_ID(inp,col,tbl):
    return DB_Fetch(dbc,f"select {col} from {tbl} where Name ='{inp}'",True,"LOE")[0]
