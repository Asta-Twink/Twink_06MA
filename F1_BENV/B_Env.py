...
Twink_path = current_path.split('\\')
project_id_with_underscore = Twink_path[3]  # Output: 12_SF
project_id_without_underscore = project_id_with_underscore.replace('_', '')  # Output: 12SF

def Run_Project(project_id_without_underscore):
    global ldir,config,Mod_Work
    file_path = fr'C:\Twink\TwinkConfig_{project_id_without_underscore}.txt'
    ...
    if config[0] =='1': Mod_Work=False; ldir = os.getcwd()
    else:Mod_Work=True; ldir = config[1]
Run_Project(project_id_without_underscore)
...
Twink_UI = uic.loadUi(fr'{ldir}\MODULES\BASE_WIN\UI-Base_Win.ui')
