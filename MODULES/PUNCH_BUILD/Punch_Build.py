...
def Punch_Build_FN(PnchBld,PrsPnchTrck):
    ...
    def Punch_Build_Push(res_list,date,reset,update): # create Punch data from Iclock Transaction
        for step in res_list:
            try:
                ...
                if reset:
                    DB_Cmt(dbc,db,fr"DELETE FROM `punch_build` WHERE (`emp_code` = '{step}') and "\
                                     fr"(`gen_date` = '{date}');",False)

                try:
                    DB_Cmt_WOE(dbc,db,fr'''INSERT INTO `punch_build` VALUES ('{step}', '{date}', 
                    ...
                    pass
                if update:
                   DB_Cmt(dbc,db,f"update attendance_track set status = 'MCU', lu_date = '{Cur_Date_SQL}' where s_date = '"
                          f"{date}' ",False)
    ...
    def PB_Daliy_Trigger():
        ...
        db_list = DB_Fetch(dbc,f'select s_date from attendance_track where status != "YTC" and month(s_date) = {mm} '
    ...
    def PTrack_Functions(Menu,event,values):
        ...
        if event == 'Ptrk_Exp':
            ...
            os.system(r'C:\Twink_08SD\Master_Files\PTRACKTemp.csv')
        ...
        if event == 'Ptrkpers_Exp':
            ...
            os.system(r'C:\Twink_08SD\Master_Files\PTRACKPERSTemp.csv')
    ...
    def Update_Punch_Build():
        ...
        for step in tempdata:
            try:
                ...
                DB_Exe(dbc,fr"DELETE FROM `punch_build` WHERE (`emp_code` = '{step[0]}') and " \
                ...
                # print(sql)
                DB_Exe(dbc,sql,False)

                DB_Cmt(dbc,db,f"update attendance_track set status = 'MNU', lu_date = '{Cur_Date_SQL}' where "
        ...
        sql = f"select punch_build.emp_code, register.employee_name, punch_build.CI, punch_build.LO, punch_build.LI, " \
                ...
                # print(DB_Fetch(dbc,sql,False,"LOL"))
        Push_Table_Values(PnchBld.OQTB_PunchTrack, DB_Fetch(dbc, sql, False, "LOL"), False)
    ...
    sql = f"select punch_build.emp_code, register.employee_name, punch_build.CI, punch_build.LO, punch_build.LI, " \
          f"punch_build.CO, punch_build.gen_attn from punch_build left join register on  punch_build.emp_code = " \
          f"register.emp_code where punch_build.gen_date = '{Cur_Date_SQL}'"
    # print(DB_Fetch(dbc,sql,False,"LOL"))
    Adjust_Table_Width(PnchBld.OQTB_PunchTrack, [100, 400, 100, 100, 100, 100, 150])
    Push_Table_Values(PnchBld.OQTB_PunchTrack,DB_Fetch(dbc,sql,False,"LOL"),False)

    PnchBld.IQDE_Date.dateChanged.connect(lambda :
    Push_Table_Values(PnchBld.OQTB_PunchTrack,DB_Fetch(dbc,
    ...
    Push_Table_Values(PrsPnchTrck.OQTB_PersonalPunchTrack,DB_Fetch(dbc,
