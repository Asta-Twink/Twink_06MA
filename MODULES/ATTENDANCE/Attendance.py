...
def Attendance_Push_FN(AttnPush):
    ...
    def Atten_Tbl_Refresh():
        Table_Data_Temp = DB_Fetch(dbc, 'select emp_code,team,employee_name,designation from register where '
                                        'active_status = "Y" order by team,employee_name', False, 'LOL')
    ...
    def Fetch_Attendance():
        if AttnPush.IQC_AttendanceFetch.isChecked():
            Table_Data_Temp = DB_Fetch(dbc, 'select emp_code,team,employee_name,designation from register where '
                        'active_status = "Y" order by team,employee_name',
                        False, 'LOL')
            ...
            for step in Table_Data_Temp:
                ...
                QS = QSpinBox();QS.setRange(1, 3);QS.setValue(int(float(Data[0][0]))
                                    if Data[0][0]!='A'and Data[0]!='NA' else 1)
                step.append(QS)
    ...
    Adjust_Table_Width(AttnPush.OQTB_Register, [100,120,250,150,50,80,80])
...
def Attendance_View_Fn(AttnView):
    ...
    def Custom_Attendance_View():
        ...
        Push_Table_Values(AttnView.OQTB_EmpDetails, View_Table_Data[0], True)
        Push_Table_Values(AttnView.OQTB_EmpAttendance, View_Table_Data[1], True)
        Adjust_Table_Width(AttnView.OQTB_EmpDetails, [100, 100, 300])
        Adjust_Table_Width(AttnView.OQTB_EmpAttendance, [80] * 30)
    ...
    def Export_Attendance_FormExport():
        ...
        for step in atndata:
            S1Data = DB_Fetch(dbc,f"select uan_no,esic_no,f_sp_name,designation,shift_work from register where "
    ...
    Adjust_Table_Width(AttnView.OQTB_EmpDetails, [100, 100, 300])
    Adjust_Table_Width(AttnView.OQTB_EmpAttendance, [100]*30)
    Push_Table_Values(AttnView.OQTB_EmpDetails,View_Table_Data[0],True)
    Push_Table_Values(AttnView.OQTB_EmpAttendance,View_Table_Data[1],True)
