...
def Attendance_View_Fn(AttnView):
    ...
    def Custom_Attendance_View():
        Atten_list = []
        employee_data = DB_Fetch("select emp_code,team,employee_name from register where "
                                 "active = 'Y'   order by UID", False, "LOL")
        sql = DB_Fetch(fr'''
        ...
        for employee in employee_data:
            values = ["NA"] * 31
            for data in sql:
                if employee[0] == data[0]:
                    if AttnView.IQCB_Attendance.currentText() == 'Attendance':
                        values[int(data[3]) - 1] = data[4].split("::")[0]
                    elif AttnView.IQCB_Attendance.currentText() == 'OT':
                        values[int(data[3]) - 1] = data[4].split("::")[1]
                    elif AttnView.IQCB_Attendance.currentText() == 'Atn+ot':
                        values[int(data[3]) - 1] = data[4]
            Atten_list.append(values)
        AttnView.OQTB_EmpDetails.horizontalHeader().setSortIndicator(0, Qt.AscendingOrder)
        Push_Table_Values(AttnView.OQTB_EmpDetails,employee_data,False)
        Push_Table_Values(AttnView.OQTB_EmpAttendance,Atten_list,False)
